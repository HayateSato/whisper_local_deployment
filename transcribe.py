import whisper
import os
import torch
from pathlib import Path
from datetime import timedelta
import time

def format_timestamp(seconds):
    """Convert seconds to [HH:MM:SS] format"""
    td = timedelta(seconds=int(seconds))
    return f"[{str(td)}]"

def transcribe_with_timestamps(model, audio_path, interval_seconds=300):
    """
    Transcribe audio with timestamp annotations
    
    Args:
        model: Loaded Whisper model
        audio_path: Path to audio file
        interval_seconds: Interval for timestamp annotations (default 300 = 5 minutes)
    """
    print(f"Transcribing: {audio_path}")
    
    # Transcribe with word-level timestamps
    result = model.transcribe(
        str(audio_path),
        language="de",  # Force German language
        word_timestamps=True,
        verbose=False
    )
    
    # Build transcription with interval timestamps
    transcription_lines = []
    last_timestamp_mark = 0
    
    for segment in result['segments']:
        segment_start = segment['start']
        segment_text = segment['text'].strip()
        
        # Add timestamp marker if we've crossed an interval
        if segment_start >= last_timestamp_mark + interval_seconds:
            timestamp_str = format_timestamp(segment_start)
            transcription_lines.append(f"\n{timestamp_str}\n")
            last_timestamp_mark = segment_start
        
        transcription_lines.append(segment_text)
    
    return ' '.join(transcription_lines)

def process_directory(base_path, output_base_path, model_size="large-v3", timestamp_interval=300):
    """
    Process all MP3 files in directory structure
    
    Args:
        base_path: Root directory containing section folders with MP3 files
        output_base_path: Root directory for output text files (mirrors input structure)
        model_size: Whisper model size (options: tiny, base, small, medium, large, large-v2, large-v3)
        timestamp_interval: Seconds between timestamp annotations (default 300 = 5 min)
    """
    
    # Check GPU availability
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"Using device: {device}")
    
    if device == "cuda":
        print(f"GPU: {torch.cuda.get_device_name(0)}")
        print(f"CUDA Version: {torch.version.cuda}")
        print(f"Available GPU Memory: {torch.cuda.get_device_properties(0).total_memory / 1e9:.2f} GB")
    
    # Load model
    print(f"\nLoading Whisper {model_size} model...")
    model = whisper.load_model(model_size, device=device)
    print("Model loaded successfully!\n")
    
    # Find all MP3 files
    base_path = Path(base_path)
    mp3_files = list(base_path.rglob("*.mp3"))
    
    if not mp3_files:
        print(f"No MP3 files found in {base_path}")
        return
    
    print(f"Found {len(mp3_files)} MP3 files to process\n")
    print("=" * 80)
    
    # Process each file
    total_start_time = time.time()
    
    for idx, mp3_path in enumerate(mp3_files, 1):
        file_start_time = time.time()
        
        # Create output path mirroring input structure
        relative_path = mp3_path.relative_to(base_path)
        output_path = Path(output_base_path) / relative_path.with_suffix('.txt')
        output_path.parent.mkdir(parents=True, exist_ok=True)
        
        # Skip if already processed
        if output_path.exists():
            print(f"[{idx}/{len(mp3_files)}] SKIPPED (already exists): {mp3_path.name}")
            continue
        
        print(f"[{idx}/{len(mp3_files)}] Processing: {relative_path}")
        
        try:
            # Transcribe
            transcription = transcribe_with_timestamps(
                model, 
                mp3_path, 
                interval_seconds=timestamp_interval
            )
            
            # Save to file
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(transcription)
            
            file_duration = time.time() - file_start_time
            print(f"    ✓ Completed in {file_duration:.1f}s → {output_path}")
            print(f"    Progress: {idx}/{len(mp3_files)} ({idx/len(mp3_files)*100:.1f}%)")
            
        except Exception as e:
            print(f"    ✗ ERROR: {str(e)}")
            # Log error to file
            error_log = output_path.with_suffix('.error.txt')
            with open(error_log, 'w', encoding='utf-8') as f:
                f.write(f"Error processing {mp3_path}:\n{str(e)}")
        
        print("-" * 80)
    
    total_duration = time.time() - total_start_time
    print(f"\n{'=' * 80}")
    print(f"COMPLETE! Total time: {total_duration/3600:.2f} hours")
    print(f"Average time per file: {total_duration/len(mp3_files):.1f}s")
    print(f"{'=' * 80}")

if __name__ == "__main__":
    # Configuration
    MP3_BASE_PATH = r"C:\path\to\your\mp3_location"  # UPDATE THIS PATH
    OUTPUT_BASE_PATH = r"C:\path\to\your\transcriptions"  # UPDATE THIS PATH
    
    # Model selection for German:
    # - "large-v3": Best quality, ~10GB VRAM (RECOMMENDED for your RTX 5090)
    # - "large-v2": Slightly older, similar quality
    # - "medium": Faster, ~5GB VRAM, good quality
    # - "small": Even faster, lower quality
    
    MODEL_SIZE = "large-v3"  # Best for German with your hardware
    
    # Timestamp interval in seconds (300 = 5 minutes)
    TIMESTAMP_INTERVAL = 300
    
    # Start processing
    process_directory(
        base_path=MP3_BASE_PATH,
        output_base_path=OUTPUT_BASE_PATH,
        model_size=MODEL_SIZE,
        timestamp_interval=TIMESTAMP_INTERVAL
    )